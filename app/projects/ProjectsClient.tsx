...
type Props = {
  /** Pré-hydratation SSR optionnelle */
...
export default function ProjectsClient({ initialItems = [] }: Props) {
  ...
  // Démarrage immédiat avec les items fournis par la page server (si présents)
  ...
  /* ========= Fetch & normalisation titres (rafraîchit côté client) ========= */
  useEffect(() => {
    let stop = false;
    (async () => {
      try {
